...
class UdpExposeClient:
    ...
    async def start_broadcasting(self):
        ...
        try:
            ...
            _logger.info(f"UDP广播客户端已启动，广播地址: {self.broadcast_address}:{self.broadcast_port}")
            
        except Exception as e:
            _logger.error(f"启动UDP广播客户端失败: {e}")
            self.running = False
            if self.sock:
                self.sock.close()
                self.sock = None
    ...
    async def _send_broadcast(self):
        ...
        try:
            ...
            _logger.info(f"已广播系统发现消息: {discovery_message}")
            ...
            if self.on_broadcast_callback:
                try:
                    ...
                    _logger.error(f"广播回调函数执行出错: {e}")
                    
        except Exception as e:
            _logger.error(f"发送广播消息失败: {e}")


udp_expose_client = UdpExposeClient()