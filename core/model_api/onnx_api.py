...
class OnnxCageHandler:
    ...
    def add_model(self, model_name: str, model_info: InitStruct) -> None:
        self.model_list[model_name] = model_info
    
    def del_model(self, model_name: str) -> None:
        if model_name in self.model_list:
            del self.model_list[model_name]
    
    def save_model(self):
        self.keys_json = self.model_list
...
class ModelDriver:
    ...
    def _warmup(self):
        try:
            ...
            _logger.info(f"Model Pre-heating done: {self.config['model_path']}, costs: {warmup_time:.2f}ms")

        except Exception as e:
            _logger.error(f"Pre-heating error: {e}")
...
class OnnxApi:
    ...
    def add_model(self, model_name: str) -> Tuple[bool, str]:
        """添加模型"""
        with self.lock:
            config = model_cage.model_list.get(model_name, None)
            if config is None:
